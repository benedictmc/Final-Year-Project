...
class PriceClassification():
    ...
    def __init__(self):
        ...
        filename = 'data_files/training/BBC.csv'
    ...
    def build_model(self, X_train, y_train, X_test, y_test):
        ...
        filepath = "BCC_Final-{epoch:02d}-{loss:.3f}"
        checkpoint = ModelCheckpoint("../models/{}.model".format(filepath, monitor='loss', verbose=1, save_best_only=True, mode='max')) 

        model.fit(X_train, y_train, epochs=70, batch_size=16, shuffle=False, callbacks=[tensorboard, checkpoint])
